...
app.post("/expense",(req,res)=>{
    ...
    db.get("SELECT total FROM wallet WHERE id=1",(e,w)=>{
        ...
        if(w.total < amount){
            return res.send({success:false,message:"không đủ tiền"});
        }

        db.run("INSERT INTO expense(category,amount,note) VALUES(?,?,?)",
            [category,amount,note]);

        db.run("UPDATE wallet SET total=total-? WHERE id=1",[amount]);
